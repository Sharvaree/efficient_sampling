...
from qm9.property_prediction.main_qm9_prop import test, test_validity
from qm9.property_prediction import main_qm9_prop
from qm9.sampling import sample_chain, sample, sample_sweep_conditional, optimize_sample
...
class DiffusionDataloader:
    def __init__(self, args_gen, model, nodes_dist, prop_dist, device, unkown_labels=False,
                 batch_size=1, iterations=200, classifier=None, model_type='noise'):
    ...
    def sample(self):
        ...
        if self.classifier is None:
            ...
            one_hot, charges, x, node_mask = optimize_sample(self.args_gen, self.device, self.model,
                                        self.dataset_info, self.prop_dist, nodesxsample=nodesxsample,
                                        context=context, classfier=self.classifier, model_type=self.model_type)
...
@hydra.main(config_path="configs/", config_name="inv_flows_config.yaml", version_base="1.2")
def main_quantitative(cfg):
    args=cfg
    # Get classifier
    ...
    if args.task == 'edm':
        ...
        loss = test_validity(classifier, 0, diffusion_dataloader, dataset_info, mean, mad, args.property, args.device, 1, args.debug_break,
                              args.save_for_vis, args.evaluate_conditional_stability_loss)
        print("Loss classifier on Generated samples: %.4f" % loss)
    elif args.task == 'optimize_prior':
        uncond_args_gen = get_args_gen(args.uncond_generators_path)
        uncond_model, _, _, _ = get_generator(args.uncond_generators_path, dataloaders,
                                                args.device, uncond_args_gen, property_norms)
        diffusion_dataloader = DiffusionDataloader(args_gen, uncond_model, nodes_dist, prop_dist,
                                                   args.device, batch_size=args.batch_size,
                                                   iterations=args.iterations, classifier=classifier, model_type=args.model_type)
        print("EDM: We evaluate the classifier on our generated samples")
        # loss = test(classifier, 0, diffusion_dataloader, mean, mad, args.property, args.device, 1, args.debug_break)
        loss = test_validity(classifier, 0, diffusion_dataloader, dataset_info, mean, mad, args.property, args.device, 1,
                              args.debug_break, args.save_for_vis, args.evaluate_conditional_stability_loss)
        print("Loss classifier on Generated samples: %.4f" % loss)
...
if __name__ == "__main__":
    ...
    if wis:
        ...
        parser.add_argument('--uncond_generators_path', type=str, default='outputs/')
        ...
        parser.add_argument('--iterations', type=int, default=20,
                            help='break point or not')
        parser.add_argument('--task', type=str, default='edm',
                            help='naive, edm, qm9_second_half, qualitative')
        parser.add_argument('--analyze', type=str, default='quant')
        parser.add_argument('--n_sweeps', type=int, default=1,
                            help='number of sweeps for the qualitative conditional experiment')
        parser.add_argument('--save_for_vis', type=eval, default=False,
                            help='number of sweeps for the qualitative conditional experiment')
        parser.add_argument('--evaluate_conditional_stability_loss', type=eval, default=False,
                            help='number of sweeps for the qualitative conditional experiment')
